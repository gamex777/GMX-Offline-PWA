(async function() {
    ...
    // The shell can leave a core and launch another game without reloading the page.
    // Runtime source files define global classes, so evaluating them twice would throw
    // duplicate-declaration errors. Share one load promise across every loader run.
    if (!window.__CSD_EJS_RUNTIME_PROMISE) {
        window.__CSD_EJS_RUNTIME_PROMISE = (async function() {
            for (let i = 0; i < scripts.length; i++) {
                await loadScript(scripts[i]);
            }
            if (!document.querySelector('link[data-csd-ejs-style="1"]')) {
                await new Promise(function(resolve) {
                    let css = document.createElement("link");
                    css.rel = "stylesheet";
                    css.href = scriptPath + "emulator.css";
                    css.dataset.csdEjsStyle = "1";
                    css.onload = resolve;
                    css.onerror = resolve;
                    document.head.appendChild(css);
                });
            }
            window.__CSD_EJS_RUNTIME_LOADED = true;
        })().catch(function(error) {
            window.__CSD_EJS_RUNTIME_PROMISE = null;
            throw error;
        });
    }
    await window.__CSD_EJS_RUNTIME_PROMISE;
    const config = {};
